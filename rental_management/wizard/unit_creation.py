...
class UnitCreation(models.TransientModel):
    ...
    property_code_prefix = fields.Char(string="Prefix", help="Prefix for Property Code")
    ...
    def action_create_property_unit(self):
        ...
        if unit_from != 'sub_project' and self.total_floors <= 0:
            return {
        ...
        if self.units_per_floor <= 0:
            return {
                ...
                'params': {
                    'type': 'info',
                    'title': _('Total Unit !'),
                    'message': _('Total unit should be greater than 1.'),
                    'sticky': False,
                }
            }
        ...
        property_static_rec = {
            'total_floor': 1 if unit_from == 'sub_project' else self.total_floors,
        ...
        for data in property_data:
            ...
            property_rec['brocher_access_token'] = secrets.token_urlsafe(12)

            property_id = self.env['property.details'].sudo().create(property_rec)
